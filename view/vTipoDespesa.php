...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) { // aqui � onde vai decorrer a chamada se houver um *request* POST
    ...
    if (method_exists('vTipoDespesa', $method)) {
        ...
        //set
        $col = new ColTipoDespesa();
        $class = new vTipoDespesa();
...
class vTipoDespesa {
    ...
    function vCadastro($dados, $files) {
        ...
        $col->set("tipo_despesa_id", $dados['id']);
        $col->set("tipo_despesa_nome", $dados['nome']);
        $col->set("tipo_despesa_obs", $dados['obs']);
        $col->set("tipo_despesa_ativado", $dados['ativado']);
...
//header('Content-Type: application/json; charset=UTF-8');
    ...
    function vListaAll($dados, $files) {
        ...
        if ($dados['where']) {
            ...
            $where = ' order by tipo_despesa_nome';
        }
    ...
    function vBuscaAll($dados, $files) {
        ...
        $where = " where tipo_despesa_nome like '%" . $dados['where'] . "%'";
